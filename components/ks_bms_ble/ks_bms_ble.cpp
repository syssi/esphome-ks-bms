...
static const uint16_t KS_BMS_NOTIFY_CHARACTERISTIC_UUID = 0xFF01;   // handle 0x13
static const uint16_t KS_BMS_NOTIFY2_CHARACTERISTIC_UUID = 0xFF03;  // handle 0x18
static const uint16_t KS_BMS_CONTROL_CHARACTERISTIC_UUID = 0xFF02;  // handle 0x15

static const uint16_t MAX_RESPONSE_SIZE = 20;

static const uint8_t KS_PKT_START = 0x55;
static const uint8_t KS_FUNCTION_REQUEST = 0x04;
static const uint8_t KS_FUNCTION_RESPONSE = 0x14;
static const uint8_t KS_PKT_END = 0xAA;

static const uint8_t KS_FRAME_TYPE_SOFTWARE_VERSION = 0x81;
static const uint8_t KS_FRAME_TYPE_HARDWARE_VERSION = 0x82;
static const uint8_t KS_FRAME_TYPE_STATUS = 0x83;
static const uint8_t KS_FRAME_TYPE_GENERAL_INFO = 0x84;
static const uint8_t KS_FRAME_TYPE_MOSFET_STATUS = 0x85;
static const uint8_t KS_FRAME_TYPE_GYRO = 0x86;
static const uint8_t KS_FRAME_TYPE_TEMPERATURES = 0x87;
static const uint8_t KS_FRAME_TYPE_CELL_VOLTAGES_1_8 = 0x88;
static const uint8_t KS_FRAME_TYPE_CELL_VOLTAGES_9_16 = 0x89;
static const uint8_t KS_FRAME_TYPE_CELL_VOLTAGES_17_24 = 0x8A;
static const uint8_t KS_FRAME_TYPE_LOCATION = 0x90;
static const uint8_t KS_FRAME_TYPE_OWNER = 0x94;

static const uint8_t KS_COMMAND_QUEUE_SIZE = 6;
static const uint8_t KS_COMMAND_QUEUE[KS_COMMAND_QUEUE_SIZE] = {
    KS_FRAME_TYPE_STATUS,       KS_FRAME_TYPE_GENERAL_INFO,      KS_FRAME_TYPE_MOSFET_STATUS,
    KS_FRAME_TYPE_TEMPERATURES, KS_FRAME_TYPE_CELL_VOLTAGES_1_8, KS_FRAME_TYPE_CELL_VOLTAGES_9_16,
    // KS_FRAME_TYPE_CELL_VOLTAGES_17_24,
};
...
void KsBmsBle::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      //      auto *char_notify2 =
      //          this->parent_->get_characteristic(KS_BMS_SERVICE_UUID,
      //          KS_BMS_NOTIFY2_CHARACTERISTIC_UUID);
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
      ...
      this->send_command_(KS_FRAME_TYPE_SOFTWARE_VERSION);
      this->send_command_(KS_FRAME_TYPE_HARDWARE_VERSION);
      break;
    }
...
void KsBmsBle::on_ks_bms_ble_data(const uint8_t &handle, const std::vector<uint8_t> &data) {
  ...
  switch (frame_type) {
    ...
    case KS_FRAME_TYPE_STATUS:
      ...
      break;
    case KS_FRAME_TYPE_GENERAL_INFO:
      this->decode_general_info_data_(data);
      break;
    case KS_FRAME_TYPE_MOSFET_STATUS:
      this->decode_mosfet_status_data_(data);
      break;
    case KS_FRAME_TYPE_TEMPERATURES:
      this->decode_temperature_data_(data);
      break;
    case KS_FRAME_TYPE_CELL_VOLTAGES_1_8:
    case KS_FRAME_TYPE_CELL_VOLTAGES_9_16:
    case KS_FRAME_TYPE_CELL_VOLTAGES_17_24:
      this->decode_cell_voltages_data_(frame_type - KS_FRAME_TYPE_CELL_VOLTAGES_1_8, data);
      break;
    case KS_FRAME_TYPE_LOCATION:
    case KS_FRAME_TYPE_LOCATION + 1:
      ESP_LOGD(TAG, "The location frame isn't supported yet");
      break;
    case KS_FRAME_TYPE_OWNER:
    case KS_FRAME_TYPE_OWNER + 1:
...
void KsBmsBle::decode_status_data_(const std::vector<uint8_t> &data) {
  ...
  this->publish_state_(this->state_of_health_sensor_, ks_get_16bit(17) * 1.0f);
...
void KsBmsBle::decode_general_info_data_(const std::vector<uint8_t> &data) {
  ...
  this->publish_state_(this->nominal_capacity_sensor_, ks_get_16bit(5) * 0.01f);

  //  7    2  0x36 0x48    Capacity remaining                Ah   0.01f  138.96 Ah
  this->publish_state_(this->capacity_remaining_sensor_, ks_get_16bit(7) * 0.01f);

  //  9    2  0x00 0x00    Cycle count                            1.0    0
  this->publish_state_(this->charging_cycles_sensor_, ks_get_16bit(9) * 1.0f);
  ...
  this->publish_state_(
      ...
      bitmask_to_string_(TEMPERATURE_PROTECTION_ERRORS, TEMPERATURE_PROTECTION_ERRORS_SIZE, ks_get_16bit(15)));

  //  17   2  0x00 0x00    Error bitmask
  this->publish_state_(this->error_bitmask_sensor_, ks_get_16bit(17) * 1.0f);
  this->publish_state_(this->errors_text_sensor_, bitmask_to_string_(ERRORS, ERRORS_SIZE, ks_get_16bit(17)));
...
void KsBmsBle::decode_mosfet_status_data_(const std::vector<uint8_t> &data) {
  ...
  this->publish_state_(this->charging_binary_sensor_, (bool) check_bit_(ks_get_16bit(3), 1));
  this->publish_state_(this->discharging_binary_sensor_, (bool) check_bit_(ks_get_16bit(3), 2));
  this->publish_state_(this->limiting_current_binary_sensor_,
                       check_bit_(ks_get_16bit(3), 16) || check_bit_(ks_get_16bit(3), 32));
...
void KsBmsBle::decode_cell_voltages_data_(const uint8_t &chunk, const std::vector<uint8_t> &data) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    float cell_voltage = ks_get_16bit((i * 2) + 3) * 0.001f;
