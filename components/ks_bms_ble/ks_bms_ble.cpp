...
static const uint8_t KS_FRAME_TYPE_STATUS_TYPE2 = 0x61;
...
void KsBmsBle::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
  ...
  switch (event) {
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
      ...
      this->send_command_(this->device_type_ == 2 ? KS_FRAME_TYPE_STATUS_TYPE2 : KS_FRAME_TYPE_STATUS);
      break;
    }
...
void KsBmsBle::on_ks_bms_ble_data(const uint8_t &handle, const std::vector<uint8_t> &data) {
  ...
  switch (frame_type) {
    ...
    case KS_FRAME_TYPE_VOLTAGE_PROTECTION:
      ESP_LOGI(TAG, "Voltage protection frame: %s", format_hex_pretty(&data.front(), data.size()).c_str());  // NOLINT
      break;
    ...
    case KS_FRAME_TYPE_CURRENT_PROTECTION:
      ESP_LOGI(TAG, "Current protection frame: %s", format_hex_pretty(&data.front(), data.size()).c_str());  // NOLINT
      break;
